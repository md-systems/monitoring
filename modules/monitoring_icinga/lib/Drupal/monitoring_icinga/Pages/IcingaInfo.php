...
class IcingaInfo {
  ...
  function configActive() {
    ...
    $output[] = array(
      ...
      '#description' => t('Active services check means Icinga server itself is responsible for contacting host to retrieve status data. Info on this page will help you
to configure this setup.'),
    ...
    $output[] = array(
      ...
      '#description' => t('Before continuing with following setup you need to have Icinga server up and running and the NRPE connector at the monitored system. Refer to
 the README.txt for a brief setup procedure.'),
    );
    ...
    $output[] = $this->objectDefinitionsOutput();
  ...
  function configPassive() {
    ...
    $output[] = $this->objectDefinitionsOutput(FALSE);
  ...
  function configPassiveWithFreshness() {
    $output = array();

    $output[] = array(
      '#theme' => 'monitoring_config_box',
      '#description' => t('This approach combines passive and active result checks. In case Icinga freshness check of a sensor will not pass it will conduct a separate
active check of the sensor.'),
    );
 ...
 the README.txt for a brief setup procedure.'),
    ...
    $output[] = $this->objectDefinitionsOutput(FALSE, TRUE);
   ...
   *   Renderable array.
   */
  protected function objectDefinitionsOutput($active_checks = TRUE, $check_freshness = FALSE) {
